...
<div id="js__scroll-to-section" class="g-container--md g-padding-y-80--xs g-padding-y-125--sm">
    @forelse ($hairtranses as $hairtrans)
    <section>
        <h3><a href="{{ route('web:hairtrans_detail', [$hairtrans->slug]) }}">{{ $hairtrans->name }}</a> <br><small style="font-size:50%;">{{ $hairtrans->created_at->toDayDateTimeString() }}</small></h3>
        <p>{{ str_limit($hairtrans->body, 200) }}</p>
    </section>
    @empty
    <p class="text-center">{{ __('There is no data here') }}</p>
    @endforelse
    <div class="text-center">
        {!! $hairtranses->appends(['search' => request()->get('search')])->links() !!}
    </div>
</div>
