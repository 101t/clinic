(function($){
	...
	window.collection_manage = function(cmd, index){
		if (cmd == "list") {
			...
			$("#addeditmodal .modal-title").html(main_trans["adddoctor"]);
			...
			$.ajax({
				...
				success: function(data){
					var value = data;
					$("#addeditmodal .modal-title").html(main_trans["editdoctor"]);
					...
					$("#addeditmodalform input[name=major]").val(value.major);
